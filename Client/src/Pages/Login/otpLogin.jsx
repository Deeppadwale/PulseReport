...
function OtpLoginPage() {
  ...
  const [otpSent, setOtpSent] = useState(false);
  const [timer, setTimer] = useState(0);
  const [isOtpStepVisible, setIsOtpStepVisible] = useState(false);
  ...
  useEffect(() => {
    if (isOtpStepVisible && inputRefs.current[0]) {
  ...
  // Handle going back to mobile step
  const handleBackToMobile = () => {
    setOtpSent(false);
    setIsOtpStepVisible(false);
  };
    ...
    <div className="min-h-screen bg-white md:bg-[#f0f2f5] flex flex-col items-center justify-center font-sans p-4">
      ...
      >
        <div className="p-8 md:p-12 flex flex-col">
          ...
          <p className="text-base md:text-lg text-[#444] max-w-[400px]">
            {otpSent
              ? `A text message with a 6-digit verification code was just sent to +91 ${mobile} on WhatsApp`
              : "with your PulseReport Account to continue to Dashboard."}
          </p>
        </div>
          ...
          <AnimatePresence mode="wait">
            {!otpSent ? (
              ...
              <motion.div
                ...
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: -20 }}
                onAnimationComplete={() => setIsOtpStepVisible(true)}
                className="space-y-8"
              >
                <div 
