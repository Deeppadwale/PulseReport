...
//             <div className="text-center">Doctor/ Hospital</div>
...
function MemberReport() {
  ...
  const [selectedReportId, setSelectedReportId] = useState("all");
  const [availableReportTypes, setAvailableReportTypes] = useState([]);
  const [selectedDetailsForEmail, setSelectedDetailsForEmail] = useState([]);
...
const handleSendEmail = async () => {
  ...
  if (selectedDetailsForEmail.length === 0) {
    showNotification("No files selected to send", "error");
    return;
  }
  ...
  try {
    ...
    const formData = new FormData();
    formData.append("email", emailFormData.email);
    formData.append("subject", emailFormData.subject || "Medical Reports");
    formData.append(
      "messagebody",
      emailFormData.messagebody ||
        "Please find the attached medical reports."
    );

    // 🔥 Fetch & attach multiple files
    for (const detail of selectedDetailsForEmail) {
      ...
      const blob = await response.blob();
      formData.append("files", blob, fileName); // 👈 SAME KEY NAME
    }
    ...
    showNotification("Email sent with multiple attachments!");
    setShowEmailModal(false);
    setSelectedDetailsForEmail([]);
                    ...
                    <div className="flex items-center justify-center gap-1">
                      {detail.uploaded_file_report ? (
                        <>
                          ...
                          <input
                            type="checkbox"
                            checked={selectedDetailsForEmail.some(
                              (d) => d.uploaded_file_report === detail.uploaded_file_report
                            )}
                            onChange={(e) => {
                              if (e.target.checked) {
                                setSelectedDetailsForEmail((prev) => [...prev, detail]);
                              } else {
                                setSelectedDetailsForEmail((prev) =>
                                  prev.filter(
                                    (d) =>
                                      d.uploaded_file_report !== detail.uploaded_file_report
                                  )
                                );
                              }
                            }}
