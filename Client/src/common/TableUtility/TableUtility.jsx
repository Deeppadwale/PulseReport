...
const DynamicTable = ({
  ...
  const [currentPage, setCurrentPage] = useState(1);
  const [sortConfig, setSortConfig] = useState(defaultSort);
  const [expandedRows, setExpandedRows] = useState(new Set(initialExpandedRows));
  ...
  // Render expander button
  const renderExpanderButton = (row, rowIndex) => {
    const rowId = row.id || rowIndex;
  ...
  const renderCellContent = (row, column, rowIndex) => {
    ...
    // Render expander column if specified
    if (column.isExpander && expandable) {
      return renderExpanderButton(row, rowIndex);
    }
  ...
  const renderExpandedRow = (row, rowIndex) => {
    ...
    if (expandableRowRenderer) {
      return expandableRowRenderer(row, rowIndex);
    }
    ...
    return (
      <div className="p-4 bg-gray-50 border-b border-gray-200">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {columns
            .filter(col => !col.isAction && !col.isExpander)
            .map((column, colIndex) => (
    ...
    <div className={`bg-white rounded-lg shadow-sm overflow-hidden ${className}`}>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              {expandable && !columns.find(col => col.isExpander) && (
                <th
                  scope="col"
                  className="px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider text-center w-12"
                >
                  {/* Empty header for expander column */}
                </th>
              )}
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {paginatedData.length > 0 ? (
              paginatedData.map((row, rowIndex) => {
                ...
                return (
                  <React.Fragment key={rowId}>
                    <tr
                      className={`hover:bg-gray-50 ${onRowClick || expandable ? 'cursor-pointer' : ''} ${isExpanded ? 'bg-gray-50' : ''}`}
                      onClick={() => handleRowClick(row, rowIndex)}
                    >
                      {expandable && !columns.find(col => col.isExpander) && (
                        <td className="px-2 py-3 text-center">
                          {renderExpanderButton(row, rowIndex)}
                        </td>
                      )}
                    ...
                    {expandable && isExpanded && (
                      <tr className={expandedRowClassName}>
                        <td 
                          colSpan={columns.length + (expandable && !columns.find(col => col.isExpander) ? 1 : 0)}
              ...
              <tr>
                <td colSpan={columns.length + (expandable && !columns.find(col => col.isExpander) ? 1 : 0)} 
...
DynamicTable.propTypes = {
  ...
  columns: PropTypes.arrayOf(
    PropTypes.shape({
      ...
      accessor: PropTypes.string.isRequired,
      isAction: PropTypes.bool,
      isExpander: PropTypes.bool, // New prop for expander column
      actionText: PropTypes.string,
      actionRenderer: PropTypes.func,
      cellRenderer: PropTypes.func,
      formatter: PropTypes.func,
  ...
  initialExpandedRows: PropTypes.arrayOf(PropTypes.oneOfType([
    PropTypes.string,
    PropTypes.number
  ])),
};
